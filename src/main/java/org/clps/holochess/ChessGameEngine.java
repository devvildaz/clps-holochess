...
public class ChessGameEngine {
   ...
   */
  public void setInitalState(ChessGameBoard board) {
    firstClick = true;
    currentPlayer = 1;
  ...
  public boolean playerHasLegalMoves(int playerNum) {
    ...
    pieces = (playerNum == 1) ? board.getAllWhitePieces() : board.getAllBlackPieces();
  ...
  private boolean selectedPieceIsValid() {
    ...
    PieceColorEnum pieceColor = currentPiece.getColorOfPiece();
    PieceColorEnum comparedColor = currentPlayer == 2 ? PieceColorEnum.BLACK : PieceColorEnum.WHITE;
    return pieceColor == comparedColor;
  }

  /**
   * Determines if the requested King is in check.
   *
   * @param checkCurrent if true, will check if the current king is in check if
   * false, will check if the other player's king is in check.
   ...
   *
   * @return int 1 or 2 for the losing play, -1 for stalemate, or 0 for a still
   * valid game.
   */
  ...
  /**
   * Given a MouseEvent from a user clicking on a square, the appropriate action
   * is determined. Actions include: moving a piece, showing the possi ble moves
   * of a piece, or ending the game after checking game conditions.
   *
